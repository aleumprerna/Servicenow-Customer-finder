...
def _page(request: Request, selected_run: int | None = None) -> str:
    ...
    <style>
      ...
      <section><h2>Upload people CSV</h2>
        <p class="muted">Required headings: person name and LinkedIn URL. Your existing <code>Name</code> and <code>Profile URL</code> export works. OpenAI web search resolves the current employer from public sources; headline text is retained as verification context.</p>
